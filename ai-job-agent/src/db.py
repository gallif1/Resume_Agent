...
def _table_columns(conn: Any, table: str) -> set[str]:
    """Return column names for a table (SQLite or PostgreSQL)."""
    if uses_postgres():
        rows = conn.execute(
            """
...
def _apply_registry_migrations(conn: Any) -> None:
    """Evolve the registry schema for multi-user CV support (SQLite + Postgres)."""
    _ensure_users_schema(conn)
    _ensure_cvs_schema(conn)


def _ensure_users_schema(conn: Any) -> None:
    """Create/migrate the users table (safe for auth login/register)."""
    now = _utc_now()

    # Ensure users table exists (Postgres may have been created before users were added).
...
def _add_cvs_column_if_missing(conn: Any, column: str, *, sqlite_type: str, pg_type: str) -> None:
    """Add a column to cvs when upgrading legacy registries."""
    if column in _table_columns(conn, "cvs"):
        return
    try:
        if uses_postgres():
            conn.execute(f"ALTER TABLE cvs ADD COLUMN IF NOT EXISTS {column} {pg_type}")
        else:
            conn.execute(f"ALTER TABLE cvs ADD COLUMN {column} {sqlite_type}")
        conn.commit()
    except Exception as exc:
        if not _is_operational_error(exc):
            raise
...
def _ensure_cvs_schema(conn: Any) -> None:
    """Migrate legacy cvs rows for multi-user support."""
    if "cvs" not in _table_names(conn):
        return

    _add_cvs_column_if_missing(
        conn,
        "user_id",
        sqlite_type="TEXT DEFAULT 'default'",
        pg_type="TEXT DEFAULT 'default'",
    )
    _add_cvs_column_if_missing(
        conn,
        "is_active",
        sqlite_type="INTEGER DEFAULT 1",
        pg_type="INTEGER DEFAULT 1",
    )
    ...
    columns = _table_columns(conn, "cvs")
    if "user_id" not in columns or "is_active" not in columns:
        return
...
def ensure_auth_schema(db_path: Path | None = None) -> None:
    """Minimal schema init for login/register — users table only, no CV migration."""
...
def _cv_tailor_versions_ddl() -> str:
    """DDL for cv_tailor_versions — Postgres rejects SQLite AUTOINCREMENT."""
    if uses_postgres():
...
def _apply_cv_match_migrations(conn: Any) -> None:
    ...
    try:
        conn.execute(_cv_tailor_versions_ddl())
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_cv_tailor_versions_cv_job
            ON cv_tailor_versions (cv_id, job_id, id DESC)
    ...
    # Intelligent Resume Tailoring — structured reports + JD snapshots.
    try:
        if uses_postgres():
            pk = "INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
        else:
        ...
        conn.execute(
            f"""
            CREATE TABLE IF NOT EXISTS tailored_resume_reports (
                id {pk},
                version_id INTEGER,
                cv_id TEXT NOT NULL,
                job_id INTEGER NOT NULL,
                ...
                jd_snapshot_text TEXT,
                language TEXT,
                report_json TEXT NOT NULL,
                change_decisions_json TEXT,
                created_at TEXT NOT NULL
            )
            """
        )
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_tailored_resume_reports_cv_job
            ON tailored_resume_reports (cv_id, job_id, id DESC)
            """
        )
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_tailored_resume_reports_hashes
            ON tailored_resume_reports (cv_id, resume_hash, jd_snapshot_hash)
            """
        )
        conn.commit()
    except Exception as exc:
...
def save_tailored_resume_report(
    *,
    cv_id: str,
    job_id: int,
    report: dict[str, Any],
    ...
    """Persist the structured Intelligent Tailoring report + JD snapshot."""
    now = _utc_now()
    ...
    with get_connection(db_path) as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO tailored_resume_reports (
                version_id, cv_id, job_id, resume_hash, jd_snapshot_hash,
                jd_snapshot_text, language, report_json, change_decisions_json,
                created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                cv_id,
                job_id,
                report.get("resume_hash"),
                report.get("jd_snapshot_hash"),
                jd_snapshot_text or report.get("jd_snapshot"),
                report.get("language"),
                payload,
                json.dumps(report.get("change_log") or [], ensure_ascii=False),
                now,
            ),
        )
        conn.commit()
        ...
        if row_id is None:
            row = conn.execute(
                """
                SELECT id FROM tailored_resume_reports
                WHERE cv_id = ? AND job_id = ?
                ORDER BY id DESC LIMIT 1
                """,
                (cv_id, job_id),
...
def get_tailored_resume_report(
    *,
    cv_id: str,
    job_id: int,
    version_id: int | None = None,
    ...
    """Fetch a structured tailor report (by report id, version id, or latest)."""
    if _sqlite_path_missing(db_path):
        return None
    with get_connection(db_path) as conn:
        if "tailored_resume_reports" not in _table_names(conn):
        ...
        if report_id is not None:
            row = conn.execute(
                "SELECT * FROM tailored_resume_reports WHERE id = ? AND cv_id = ?",
                (report_id, cv_id),
            ).fetchone()
        elif version_id is not None:
            row = conn.execute(
                """
                SELECT * FROM tailored_resume_reports
        ...
        try:
            ...
            data["report"] = {}
        try:
            data["change_decisions"] = json.loads(
                data.get("change_decisions_json") or "[]"
            )
        except json.JSONDecodeError:
            data["change_decisions"] = []
        return data


def list_tailored_resume_reports(
    cv_id: str,
    job_id: int,
    *,
    db_path: Path = DB_PATH,
    ...
    with get_connection(db_path) as conn:
        ...
        rows = conn.execute(
            """
            SELECT id, version_id, cv_id, job_id, resume_hash, jd_snapshot_hash,
                   language, created_at
            FROM tailored_resume_reports
            WHERE cv_id = ? AND job_id = ?
            ORDER BY id DESC
            """,
            (cv_id, job_id),
...
def update_tailored_report_decisions(
    report_id: int,
    *,
    cv_id: str,
    report: dict[str, Any],
    ...
    """Persist accept/reject decisions and the resulting report document."""
    with get_connection(db_path) as conn:
        if "tailored_resume_reports" not in _table_names(conn):
            return False
        cursor = conn.execute(
            """
            UPDATE tailored_resume_reports
            SET report_json = ?, change_decisions_json = ?
            WHERE id = ? AND cv_id = ?
            """,
            (
                json.dumps(report, ensure_ascii=False),
                json.dumps(change_decisions, ensure_ascii=False),
                report_id,
                cv_id,
            ),
        )

...
class StockMarketProvider(MarketDataProvider):
    ...
    def get_candles(self, symbol: str, timeframe: str, limit: int = 100) -> list[Candle]:
        ...
        try:
            candles = self._yahoo_candles(symbol, timeframe, limit)
            if candles:
                return candles
        except Exception as exc:  # noqa: BLE001
        ...
        if self.configured:
            try:
                return self._finnhub_candles(symbol, timeframe, limit)
            except Exception as exc:  # noqa: BLE001
                logger.warning("Finnhub candles failed %s: %s", symbol, exc)
        return []

    def _yahoo_candles(self, symbol: str, timeframe: str, limit: int) -> list[Candle]:
    ...
    def _finnhub_candles(self, symbol: str, timeframe: str, limit: int) -> list[Candle]:

...
## EC2 — איך הקוד של השרת מתעדכן (חשוב)

הדיפלוי ל-EC2 (`.github/workflows/deploy.yml`) **לא בונה מחדש את ה-Docker image** —
בנייה מלאה על המכונה הקטנה תוקעת את ה-SSH. במקום זה הוא מזריק את הקוד לקונטיינר
הרץ:

1. `ai-job-agent/src` **כולו** נדחס ומועתק ל-`/app/ai-job-agent/src` בקונטיינר,
   ביחד עם `requirements.txt`. פעם היה מועתק רק קובץ-שניים ("hotfix"), וכך תיקוני
   backend נשארו ב-master בלי להגיע לפרודקשן בזמן שהדיפלוי דיווח על הצלחה.
2. לפני restart נבדק ש-`import api_server` עובד בתוך הקונטיינר. אם לא — מותקנים
   `requirements.txt`, ואם עדיין נכשל הקוד הקודם מוחזר והדיפלוי נכשל בקול.
3. אחרי restart הדיפלוי משווה את ה-revision שנשלח מול
   `/api/health` → `backend_revision`. אי-התאמה = דיפלוי נכשל, גם אם האתר עלה.

לכן `/api/health` מחזיר גם:

```json
{ "backend_revision": "<git sha>", "tailor_prompt_version": "v2" }
```

זו הדרך המהירה לבדוק איזה קוד באמת רץ: אם ה-sha אינו ה-commit האחרון ב-master,
השרת מריץ קוד ישן.

...
## סריקה על המחשב שלך (מומלץ)

הדפדפן בטלפון **לא יכול** לגשת ישירות לאתרי משרות (דרושים, LinkedIn) — זו מגבלת אבטחה של הדפדפן (CORS).
לכן סריקה «על המכשיר» בפועל = **הרצה על המחשב שלך** (Windows/Mac/Linux), והאתר בענן רק מציג תוצאות.

### אפשרות א — הכל מקומי (הכי פשוט)

```bash
./scripts/share-dev.sh
```

פותח Backend + Frontend על המחשב, ויוצר קישור לפלאפון. **כל הסריקה רצה על המחשב** — אין עומס על Render.

### אפשרות ב — UI בענן, סריקה על המחשב

1. העלה קו"ח לאתר ב-Render.
2. בחר «הרץ סוכן» → לשונית **«מחשב שלי»** → **הכן סריקה מקומית**.
3. על המחשב (clone של הפרויקט + `pip install -r ai-job-agent/requirements.txt`):
   ```bash
   python ai-job-agent/scripts/local_collect.py \
     --cv-id YOUR_CV_ID \
     --api-url https://resume-agent-xxxx.onrender.com \
     --sites drushim,linkedin
   ```
4. רענן את האתר — ההתאמות יופיעו.

השרת מבצע רק ניתוח קו"ח + חישוב התאמות (קל). איסוף המשרות רץ על המחשב שלך.
